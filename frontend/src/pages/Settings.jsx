...
import { getZohoStatus, getZohoConnectUrl, disconnectZoho, triggerZohoSync, getGmailStatus, getGmailConnectUrl, disconnectGmail } from "../api/integrations";
...
function GmailIntegrationCard() {
  ...
  useEffect(() => {
    ...
    if (gmailParam === "connected") {
      toast.success("Gmail connected! You can now import email threads.");
  ...
  const connectMutation = useMutation({
    mutationFn: () => getGmailConnectUrl().then((r) => r.data.auth_url),
    onSuccess: (authUrl) => {
      window.location.href = authUrl;
    },
    onError: (err) => {
      const msg = err.response?.data?.error || "Gmail is not configured on this server.";
      toast.error(msg);
    },
  });
    ...
    <div className="card p-6">
      <div className="flex items-start justify-between mb-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Import email threads from Gmail for AI analysis.
          </p>
        </div>
        {isLoading ? (
          <Loader2 size={18} className="animate-spin text-slate-400" />
      ...
      {connected && (
        <div className="bg-slate-50 rounded-lg px-4 py-3 mb-4 text-sm">
          {statusRes.gmail_email && (
            <p className="text-slate-600">
              <span className="font-medium">Account:</span> {statusRes.gmail_email}
            </p>
          )}
        </div>
      )}
      ...
      <div className="mt-4 border-t border-slate-100 pt-4">
        <p className="text-xs text-slate-400 font-medium mb-2">What this enables:</p>
        <ul className="text-xs text-slate-500 space-y-1">
          <li>→ Import Gmail email threads directly when creating a conversation</li>
          <li>← AI analyzes thread content and extracts sales insights</li>
        </ul>
      </div>
    </div>
  );
